...
const PostDetailScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      // Send notification if liking (not unliking) and it's not the user's own post
      if (!wasLiked && res.ok && post && post.user_id && post.user_id !== currentUserId) {
        console.log('📬 Preparing to send like notification (PostDetail):', {
          postId: post.id,
        ...
        await pushNotification({
          userId: post.user_id, // Notify the post author
          type: 'like',
          title: 'New Like on Your Post',
          body: `${currentUsername || 'Someone'} liked your post: "${post.title.substring(0, 40)}${post.title.length > 40 ? '...' : ''}"`,
          navigateTo: 'PostDetail',
          navigateParams: { postId: post.id },
        });
        console.log('✅ Like notification sent (PostDetail)');
      }
    } catch (err) {
      setLiked(wasLiked);
      setLikeCount((prev) => prev + (wasLiked ? 1 : -1));
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      // Send notification for comment or reply
      if (res.ok) {
        const currentUsername = await AsyncStorage.getItem('username');
        
        if (replyingToCommentId) {
          // It's a reply - notify the original comment author
          const originalComment = post?.comments.find(c => c.id === replyingToCommentId);
          if (originalComment && originalComment.author_id && originalComment.author_id !== currentUserId) {
            await pushNotification({
              userId: originalComment.author_id, // Notify the comment author
              type: 'reply',
              title: 'New Reply to Your Comment',
              body: `${currentUsername || 'Someone'} replied: "${commentText.trim().substring(0, 50)}${commentText.trim().length > 50 ? '...' : ''}"`,
              navigateTo: 'PostDetail',
              navigateParams: { postId },
            });
          }
        } else if (post && post.user_id && post.user_id !== currentUserId) {
          // It's a comment on the post - notify the post author
          await pushNotification({
            userId: post.user_id, // Notify the post author
            type: 'comment',
            title: 'New Comment on Your Post',
            body: `${currentUsername || 'Someone'} commented: "${commentText.trim().substring(0, 50)}${commentText.trim().length > 50 ? '...' : ''}"`,
            navigateTo: 'PostDetail',
            navigateParams: { postId },
          });
        }
  ...
  const handleLikeComment = async (commentId: string, commentIndex: number) => {
    ...
    try {
      ...
      // Send notification if liking (not unliking) and it's not the user's own comment
      if (!wasLiked && res.ok) {
        const comment = post?.comments.find(c => c.id === commentId);
        if (comment && comment.author_id && comment.author_id !== currentUserId) {
          const currentUsername = await AsyncStorage.getItem('username');
          await pushNotification({
            userId: comment.author_id, // Notify the comment author
            type: 'like',
            title: 'New Like on Your Comment',
            body: `${currentUsername || 'Someone'} liked your comment: "${comment.content.substring(0, 50)}${comment.content.length > 50 ? '...' : ''}"`,
            navigateTo: 'PostDetail',
            navigateParams: { postId },
          });
        }
      ...
      >
        ...
        <View style={styles.header}>
          <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
            <Ionicons name="chevron-back" size={26} color="#2e4420" />
          </TouchableOpacity>
          ...
          {isOwnPost ? (
            <View style={styles.headerActions}>
              ...
              >
                <Ionicons name="create-outline" size={18} color="#3d6b22" />
              </TouchableOpacity>
        ...
        >
          {/* ── White post card (mirrors commentCard styling) ── */}
