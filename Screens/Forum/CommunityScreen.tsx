...
const CommunityScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleLike = async (postId: string) => {
    ...
    try {
      ...
      if (!alreadyLiked && res.ok) {
        const post = posts.find(p => p.id === postId);
        if (post && post.user_id && post.user_id !== currentUserId) {
          console.log('📬 Preparing to send like notification:', {
            postId: post.id,
          ...
          console.log('⏭️ Skipping notification - own post or missing user_id:', {
            hasPost: !!post,
