...
const HomeScreen: React.FC<Props> = ({ navigation }) => {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      // Main content below hero fades + scales in with delay
      Animated.sequence([
        Animated.delay(300),
        Animated.parallel([
  ...
  const handleAction = (screen: keyof RootStackParamList) => {
    if (screen === 'Chatbot') {
      // Direct to floating chatbot
      navigation.navigate('FloatingChatbot' as any);
      return;
    }
    if (screen === 'Analytics') {
      // Must be logged in first
      navigation.navigate('Login' as any, { redirectTo: 'Analytics' } as any);
      return;
    }
  ...
  const dynamicStyles = StyleSheet.create({
    ...
    // Horizontal scrollable image strip for forum posts
    forumImageStrip: {
      marginBottom: 10,
    },
    forumImageThumb: {
      ...
      >
        ...
        <View style={dynamicStyles.heroSection}>
          ...
          >
            ...
            {/* Hero Content — animated slide up + fade in */}
            <Animated.View
              style={[
                styles.heroContent,
                {
        ...
        {/* Main Content Section — animated scale + fade in */}
        <Animated.View
          style={[
                  ...
                  {recentPosts.length > 0 ? (
                    recentPosts.map((post) => (
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          (navigation as any).navigate('CommunityStack', {
                            screen: 'PostDetail',
                      ...
                      >
                        ...
                        {post.imageUrls && post.imageUrls.length > 0 && (
                          <ScrollView
                            ...
                            style={dynamicStyles.forumImageStrip}
                          >
                            {post.imageUrls.map((url, imgIdx) => (
                              <View key={imgIdx} style={{ position: 'relative' }}>
                                <Image
                                  source={{ uri: url }}
                                  style={dynamicStyles.forumImageThumb}
                                  resizeMode="cover"
                                />
                              </View>
                            ))}
