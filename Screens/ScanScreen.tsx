...
// // ─────────────────────────────────────────────────────────────
// // Sub-components  (bounding-box overlays kept for leaf + disease)
// // ─────────────────────────────────────────────────────────────
...
//             position: 'absolute', left: xp*sx+m.ox, top: yp*sy+m.oy,
//             width: wp*sx, height: hp*sy,
//             borderColor: leafColors[det.class] || '#999', borderWidth: idx===0 ? 3 : 2,
...
//             position: 'absolute', left: xp*sx+m.ox, top: yp*sy+m.oy,
//             width: wp*sx, height: hp*sy,
//             borderColor: diseaseColors[det.class] || '#999', borderWidth: idx===0 ? 3 : 2,
//             zIndex: detections.length - idx,
//           }]}>
...
//   const [facing, setFacing] = useState<CameraFacing>(CameraFacing.back);
//   const [saving,        setSaving]        = useState(false);
//   const [rejectionInfo, setRejectionInfo] = useState<{class: string; confidence: number} | null>(null);
//   const cameraRef = useRef<CameraView>(null);
//   const analyzingRef = useRef(false); // Prevent double-calls
...
//   const switchCamera = () => { setFacing(prev => prev === CameraFacing.back ? CameraFacing.front : CameraFacing.back);};
...
//   // ── Core analysis — 3-stage pipeline ─────────────────────
...
//       // ────────────────────────────────────────────────────────
//       // FRUIT tab — 3-stage pipeline
//       // ────────────────────────────────────────────────────────
//       if (activeTab === 'FRUIT') {

//         // ── Stage 0: avocado classification gate ────────────
//         setProgressLabel('Stage 1/3 — Checking if image contains an avocado…');
//         setProgress(10);

//         const avocadoCheck = await fruitDiseaseApi.checkIsAvocado(img);

//         if (!avocadoCheck.success) {
//           throw new Error(avocadoCheck.error || 'Avocado classification failed');
//         }

//         if (!avocadoCheck.isAvocado) {
//           // Not an avocado — show rejection with visual feedback
//           console.log(`❌ Rejected: Not an avocado (${avocadoCheck.class})`);
//           analyzingRef.current = false;
//           setAnalyzing(false);
//           setProgress(0);
//           setProgressLabel('');
//           setRejectionInfo({
//             class: avocadoCheck.class,
//             confidence: avocadoCheck.confidence
//           });
//           Alert.alert(
//             '🚫 Not an Avocado',
//             `The image appears to contain "${avocadoCheck.class}" (${(avocadoCheck.confidence * 100).toFixed(1)}% confidence).\n\nPlease capture an image of an avocado to proceed.`,
...
//         // ── Stage 1 + 2: ripeness & disease in parallel ─────
//         setProgressLabel('Stage 2/3 — Analysing ripeness…');

//         const [ripenessApiResult, diseaseApiResult] = await Promise.all([
//           ripenessApi.predictRipeness(img),
//           (() => {
//             setProgressLabel('Stage 2-3/3 — Analysing ripeness & disease…');
//             return fruitDiseaseApi.predictFruitDisease(img);
//           })(),
//         ]);
...
//           throw new Error(ripenessApiResult.error || 'Ripeness analysis failed');

//         if (!diseaseApiResult.success || !diseaseApiResult.prediction)
//           throw new Error(diseaseApiResult.error || 'Disease analysis failed');
...
//           // Gate result
//           isAvocado       : true,
//           avocadoClass    : avocadoCheck.class,
//           avocadoConfidence: avocadoCheck.confidence,
//           // Ripeness
...
//         setProgressLabel('Analysing leaf / stem…');
//         setProgress(30);
...
//   // ── Guards ────────────────────────────────────────────────
//   if (!permission)
//     return <View style={styles.centerContainer}><ActivityIndicator size="large" color="#4CAF50" /></View>;
...
//                 {activeTab === 'FRUIT' ? 'Position avocado in frame' : 'Position leaf/stem in frame'}
//               </Text>
...
//                 {activeTab === 'FRUIT'
//                   ? 'Scan an avocado — the app first confirms it\'s an avocado, then checks ripeness and disease.'
...
//         {/* ══════════════════════════════════════════════════════
//             REJECTION NOTICE (NOT AVOCADO)
//         ══════════════════════════════════════════════════════ */}
//         {rejectionInfo && capturedImage && !result && (
//           <View style={[styles.resultCard, { borderColor: '#F44336', borderWidth: 2 }]}>
//             <View style={[localStyles.gateBadge, { backgroundColor: '#F44336' }]}>
//               <Ionicons name="close-circle" size={18} color="#fff" />
//               <Text style={localStyles.gateBadgeText}>
//                 🚫 Not an Avocado
//               </Text>
//             </View>

//             <View style={[styles.ripenessIndicator, { backgroundColor: '#F44336' }]}>
//               <Text style={styles.ripenessLevel}>❌ REJECTED</Text>
//               <Text style={styles.ripenessDescription}>
//                 Image contains "{rejectionInfo.class}" (not avocado)
//               </Text>
//             </View>

//             <View style={{ padding: 16, backgroundColor: '#FFF3E0' }}>
//               <Text style={{ fontSize: 14, color: '#E65100', textAlign: 'center', marginBottom: 8 }}>
//                 <Ionicons name="information-circle" size={16} /> Detection Confidence: {(rejectionInfo.confidence * 100).toFixed(1)}%
//               </Text>
//               <Text style={{ fontSize: 13, color: '#666', textAlign: 'center' }}>
//                 This image was classified as "{rejectionInfo.class}" and cannot be analyzed for avocado ripeness or disease.
//               </Text>
//             </View>
...
//             {/* Avocado gate confirmation badge */}
//             <View style={localStyles.gateBadge}>
//               <Ionicons name="checkmark-circle" size={18} color="#fff" />
//               <Text style={localStyles.gateBadgeText}>
//                 ✅ Avocado confirmed ({((result.avocadoConfidence ?? 0) * 100).toFixed(1)}% confidence)
//               </Text>
//             </View>

//             {/* Ripeness indicator */}
//             <View style={[styles.ripnessIndicator, {
//               backgroundColor: RIPENESS_COLORS[result.ripeness] || '#4CAF50',
//             }]}>
//               <Text style={styles.ripenessLevel}>
//                 {RIPENESS_EMOJI[result.ripeness]} {result.ripeness?.toUpperCase()}
//               </Text>
//               <Text style={styles.ripenessDescription}>
//                 {RIPENESS_DESCRIPTIONS[result.ripeness]}
//               </Text>
//             </View>

//             {/* Image */}
//             <View style={styles.comparisonContainer}>
//               <Text style={styles.comparisonTitle}>🥑 Scanned Avocado</Text>
//               <View style={styles.imageComparisonRow}>
//                 <View style={[styles.imageComparisonItem, { flex: 1 }]}>
...
//                 <Text style={styles.resultLabel}>RIPENESS</Text>
//                 <Text style={[styles.resultValue, { color: RIPENESS_COLORS[result.ripeness] || '#333' }]}>
//                   {result.ripeness?.toUpperCase()}
//                 </Text>
//               </View>
//               <View style={styles.resultRow}>
...
//               </View>
//               <View style={styles.resultRow}>
//                 <Text style={styles.resultLabel}>DAYS TO RIPE</Text>
//                 <Text style={styles.resultValue}>{result.days_to_ripe}</Text>
//               </View>
//               <View style={styles.resultRow}>
//                 <Text style={styles.resultLabel}>CONFIDENCE</Text>
//                 <Text style={styles.resultValue}>{((result.confidence ?? 0) * 100).toFixed(1)}%</Text>
//               </View>
//             </View>

//             {/* Confidence bar */}
//             <View style={[localStyles.finalConfidenceRow, {
//               backgroundColor: `${RIPENESS_COLORS[result.ripeness] || '#999'}15`,
//               borderColor    : `${RIPENESS_COLORS[result.ripeness] || '#999'}40`,
//             }]}>
//               <Text style={localStyles.finalConfidenceLabel}>Ripeness Confidence</Text>
...
//                   backgroundColor: RIPENESS_COLORS[result.ripeness] || '#999',
//                 }]} />
//               </View>
//               <Text style={[localStyles.finalConfidenceValue, {
//                 color: RIPENESS_COLORS[result.ripeness] || '#333',
//               }]}>
...
//                 <Text style={styles.probabilityTitle}>📊 Ripeness Probabilities</Text>
...
//                         backgroundColor: RIPENESS_COLORS[cls] || '#999',
//                       }]} />
//                     </View>
...
//                     Disease Analysis: {result.fruitDiseaseDetections.length} Region(s) Scanned
//                   </Text>
//                 </View>

//                 {result.image_size && (
//                   <View style={styles.comparisonContainer}>
//                     <Text style={styles.comparisonTitle}>🔬 Disease Detection</Text>
//                     <View style={styles.imageComparisonRow}>
...
//                   <Text style={styles.detectionsListTitle}>🩺 Disease Analysis</Text>
...
//                             .sort(([clsA, a], [clsB, b]) =>
//                               clsA === det.class ? -1 : clsB === det.class ? 1
...
//         {/* ══════════════════════════════════════════════════════
//             LEAF RESULT CARD  (unchanged logic)
//         ══════════════════════════════════════════════════════ */}
...
//             {activeTab === 'FRUIT'
//               ? 'The app verifies it\'s an avocado first, then analyses ripeness and disease.'
