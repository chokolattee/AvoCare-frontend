...
import {
  ...
  CHAT_MIN_HEIGHT,
  CHAT_MAX_HEIGHT,
} from '../Styles/Floatingchatbot.styles';
  ...
  const renderInline = (text: string, key: string) => {
    ...
    return (
      <Text key={key}>
        {parts.map((part, i) =>
          /^\*\*(.+)\*\*$/.test(part) ? (
            <Text key={i} style={chatStyles.mdBold}>
              {part.slice(2, -2)}
            </Text>
          ) : (
  ...
  return (
    <View>
      {content.split('\n').map((line, i) => {
        ...
        if (/^#{1,3}\s/.test(line)) {
          ...
          return (
            <Text
              key={i}
              style={[
                chatStyles.mdHeading,
                level === 1 ? chatStyles.mdH1 : level === 2 ? chatStyles.mdH2 : chatStyles.mdH3,
              ]}
            >
              {line.replace(/^#+\s/, '')}
            </Text>
        ...
        if (/^[*\-•]\s/.test(line)) {
          return (
            <View key={i} style={chatStyles.mdBulletRow}>
              <Text style={chatStyles.mdBulletIcon}>🌿 </Text>
              <Text style={chatStyles.mdBulletText}>
        ...
        if (/^\d+\.\s/.test(line)) {
          ...
          return (
            <View key={i} style={chatStyles.mdBulletRow}>
              <Text style={chatStyles.mdBulletIcon}>{num}. </Text>
              <Text style={chatStyles.mdBulletText}>
                {renderInline(line.replace(/^\d+\.\s/, ''), `n-${i}`)}
              </Text>
            </View>
          );
        }
        return (
          <Text key={i} style={chatStyles.mdParagraph}>
...
// ─── Resize Handle (Mobile — PanResponder) ────────────────────────────────────
...
const ResizeHandle: React.FC<ResizeHandleProps> = ({ chatHeight, onHeightChange }) => {
  const [isActive, setIsActive] = useState(false);
  const startHeightRef = useRef(chatHeight);
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_: GestureResponderEvent, gestureState: PanResponderGestureState) => {
        // Dragging up (negative dy) makes window taller
        const newHeight = Math.max(
          CHAT_MIN_HEIGHT,
          Math.min(CHAT_MAX_HEIGHT, startHeightRef.current - gestureState.dy)
        );
  ...
  if (Platform.OS === 'web') return null; // web uses its own handler

  return (
    <View
      {...panResponder.panHandlers}
      style={[chatStyles.resizeHandle, isActive && chatStyles.resizeHandleActive]}
    >
      <View style={chatStyles.resizeHandlePill} />
    </View>
  );
};

// ─── Web Resize Handle ────────────────────────────────────────────────────────
// On web we attach native pointer events directly to a div wrapper

const WebResizeHandle: React.FC<ResizeHandleProps> = ({ chatHeight, onHeightChange }) => {
  const [isActive, setIsActive] = useState(false);
  const startYRef = useRef(0);
  const startHeightRef = useRef(chatHeight);

  // Keep current chatHeight accessible in event listeners
  ...
  const onMouseDown = useCallback((e: any) => {
    ...
    startHeightRef.current = chatHeightRef.current;
    setIsActive(true);

    const onMouseMove = (ev: any) => {
      const dy = ev.clientY - startYRef.current;
      // dragging up = ev.clientY < startY = negative dy → taller
      const newHeight = Math.max(
        CHAT_MIN_HEIGHT,
        Math.min(CHAT_MAX_HEIGHT, startHeightRef.current - dy)
      );
      onHeightChange(newHeight);
    };
...
const FloatingChatbot: React.FC<FloatingChatbotProps> = ({
  ...
  useEffect(() => {
    Animated.spring(badgeScaleAnim, {
      toValue: unreadCount > 0 && !isOpen ? 1 : 0,
  ...
  useEffect(() => {
    setChatMessages([
      { id: '1', text: WELCOME_MESSAGES[language], isUser: false, timestamp: new Date() },
    ]);
  }, [language]);
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === 'AbortError')
          return { text: 'Request timed out. Please check your connection.', isError: true };
        if (error.message.includes('Network request failed'))
          return { text: 'Cannot connect to server.', isError: true };
      }
  ...
  const handleSendMessage = async () => {
    ...
    setChatMessages(prev => [
      ...prev,
      { id: (Date.now() + 1).toString(), text, isUser: false, timestamp: new Date(), isError },
    ]);
    setIsTyping(false);
  ...
  const chatTransform = [
    ...
    { translateY: scaleAnim.interpolate({ inputRange: [0, 1], outputRange: [chatHeight / 2, 0] }) },
  ];
  ...
  return (
    <>
      ...
      >
        {/* ── Resize Handle (top) ── */}
        {Platform.OS === 'web' ? (
          <WebResizeHandle chatHeight={chatHeight} onHeightChange={setChatHeight} />
        ) : (
          <ResizeHandle chatHeight={chatHeight} onHeightChange={setChatHeight} />
        )}
          ...
          <View style={chatStyles.leavesContainer} pointerEvents="none">
            {(['🍃', '🌿', '🍃', '🌿'] as const).map((leaf, i) => (
              <Text
                key={i}
                style={[
                  chatStyles.leafDecor,
                  chatStyles[`leaf${i + 1}` as keyof typeof chatStyles] as any,
                ]}
              >
                {leaf}
              </Text>
          ...
          <View style={chatStyles.headerAvoRow}>
            ...
            >
              <View style={chatStyles.avoAvatarWrap}>
                <Animated.Text
                  style={[
                    chatStyles.avoAvatarEmoji,
                    { transform: [{ translateY: avoBobTranslate }, { rotate: avoBobRotate }] },
                  ]}
                >
                  🥑
                </Animated.Text>
                {[0, 1, 2, 3, 4, 5].map(i => (
                  <FloatingHeart key={i} play={heartsPlaying} index={i} />
                ))}
              </View>
            </TouchableOpacity>
        ...
        >
          {chatMessages.map(msg => (
            <View
              key={msg.id}
              style={[chatStyles.msgRow, msg.isUser ? chatStyles.msgRowUser : chatStyles.msgRowBot]}
            >
              {!msg.isUser && (
                <View style={chatStyles.msgAvatar}>
                  <Text style={chatStyles.msgAvatarEmoji}>🥑</Text>
                </View>
              )}
              <View
                style={[
                  chatStyles.bubble,
                  msg.isUser ? chatStyles.bubbleUser : chatStyles.bubbleBot,
                  msg.isError && chatStyles.bubbleError,
                ]}
              >
                {msg.isUser ? (
              ...
              {msg.isUser && (
                <View style={[chatStyles.msgAvatar, chatStyles.msgAvatarUser]}>
          ...
          {isTyping && (
            <View style={[chatStyles.msgRow, chatStyles.msgRowBot]}>
              ...
              <View style={[chatStyles.bubble, chatStyles.bubbleBot, { paddingVertical: 12 }]}>
                <TypingIndicator />
              </View>
            </View>
          )}
        ...
        <View style={chatStyles.inputArea}>
          ...
          <TouchableOpacity
            style={[
              chatStyles.quickToggleBtn,
              showQuickQuestions && chatStyles.quickToggleBtnActive,
            ]}
          ...
          <TextInput
            ...
            placeholder={
              language === 'english'
                ? 'Ask about avocado farming...'
                : language === 'filipino'
                ? 'Magtanong tungkol sa abokado...'
                : 'Magtanong about avocado farming...'
            }
          ...
          <TouchableOpacity
            style={[
              chatStyles.sendBtn,
              (currentMessage.trim() === '' || isTyping) && chatStyles.sendBtnDisabled,
            ]}
          ...
          >
            {isTyping ? (
              ...
              <Text style={chatStyles.sendBtnIcon}>➤</Text>
            )}
          </TouchableOpacity>
        </View>
      </Animated.View>
