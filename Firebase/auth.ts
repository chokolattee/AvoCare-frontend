...
export const signInWithGoogle = async (): Promise<User | null> => {
  try {
    ...
    const result: UserCredential = await signInWithPopup(auth, provider, browserPopupRedirectResolver);
    const user = result.user;
    
    console.log('✅ Google Sign-In successful:', user.email);
    
    // Fire-and-forget: don't await so an ad-blocker blocking Firestore
    // can't stall or break the sign-in flow.
    storeUserInFirestore(user, 'google').catch(() => {});
...
/**
 * Sign in with Google using a credential (id_token) obtained from expo-auth-session.
 * Use this on native (iOS/Android) instead of signInWithPopup.
 * @param idToken The Google id_token from expo-auth-session response
 * @returns Firebase User object or null
 */
export const signInWithGoogleCredential = async (idToken: string): Promise<User | null> => {
  try {
    const credential = GoogleAuthProvider.credential(idToken);
    const result: UserCredential = await signInWithCredential(auth, credential);
    const user = result.user;

    console.log('✅ Google Credential Sign-In successful:', user.email);
    // Fire-and-forget so an ad-blocker blocking Firestore can't stall sign-in.
    storeUserInFirestore(user, 'google').catch(() => {});
    return user;
  } catch (error: any) {
...
export const createUserWithEmail = async (
  ...
  try {
    ...
    // Fire-and-forget Firestore storage (ad-blockers can block this)
    storeUserInFirestore(user, 'email').catch(() => {});
